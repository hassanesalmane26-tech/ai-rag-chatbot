...
import { Brain, LoaderCircle, Power, RefreshCw, Trash2 } from "lucide-react";
...
export default function MemoryView() {
  ...
  const { memories, loading, mutationId, error, refresh, create, toggle, remove } = useWorkspaceMemories(activeWorkspaceId);
  ...
  async function submit(event) {
    ...
    setValidation(issue);
    if (issue) return;
    if (await create({ title: title.trim(), content: content.trim(), kind })) {
  ...
  return <section className="memory-view" aria-labelledby="memory-title">
    ...
    <form className="memory-composer" onSubmit={submit}>
      ...
      <button type="submit" disabled={mutationId === "create"}>{mutationId === "create" ? <LoaderCircle className="spin" size={17} /> : <Brain size={17} />} Mémoriser</button>
    </form>
