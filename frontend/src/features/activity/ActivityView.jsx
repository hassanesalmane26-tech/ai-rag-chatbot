...
export default function ActivityView() {
  ...
  const { events, loading, error, lifecycle, refresh } = useWorkspaceActivity(activeWorkspaceId);
  return <section className="product-view activity-view" aria-labelledby="activity-title" data-activity-state={lifecycle} aria-busy={loading}>
    ...
    {loading ? <div className="document-loading" aria-live="polite"><LoaderCircle className="spin" /> Chargement de l’activité…</div> : error ? <div className="inline-error" role="alert"><span>{error}</span><button type="button" onClick={refresh}>Réessayer</button></div> : events.length === 0 ? <div className="empty-state"><Activity size={30} /><h3>Le Workspace attend sa première action</h3><p>Les conversations, sources Knowledge et mémoires apparaîtront ici.</p></div> : <ol className="activity-timeline">{events.map((event) => <li key={event.id}><span className="activity-timeline__mark" aria-hidden="true" /><div><strong>{event.label}</strong><span>{formatActivityTime(event.created_at)}</span></div></li>)}</ol>}
  </section>;
}
