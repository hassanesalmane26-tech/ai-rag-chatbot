...
export default function WorkspaceHome() {
  ...
  return <section className="workspace-home workspace-home--definitive">
    ...
    <GlassPanel className="workspace-hero" elevated>
      <div className="workspace-hero__scene" aria-hidden="true"><i /><i /><i /></div>
      <div className="workspace-hero__copy"><span>TRIDENT AI · WORKSPACE INTELLIGENT</span><h2>{activeWorkspace?.name || "Chargement du Workspace"}</h2><p>{activeWorkspace?.description || "Votre environnement central pour travailler avec Nova, structurer Knowledge et piloter Memory."}</p><div className="hero-actions" aria-label="Actions essentielles du Workspace"><Button onClick={() => setActiveView("conversations")}><MessageSquare size={17} /><span><strong>Ouvrir Nova</strong><small>Interroger le Workspace</small></span></Button><Button variant="secondary" onClick={() => setActiveView("knowledge")}><BookOpen size={17} /><span><strong>Knowledge</strong><small>Ajouter une source</small></span></Button><Button variant="secondary" onClick={() => setActiveView("memory")}><Brain size={17} /><span><strong>Memory</strong><small>Gérer les repères</small></span></Button><Button variant="secondary" onClick={() => setActiveView("files")}><Files size={17} /><span><strong>Fichiers</strong><small>Ouvrir les fichiers</small></span></Button></div></div>
      <aside className="workspace-core" aria-label="Systèmes connectés du Workspace"><div className="workspace-core__field" aria-hidden="true"><i /><i /><i /></div><div className="workspace-core__identity"><TridentMark /><span>INTELLIGENT CORE</span><strong>Workspace actif</strong></div><div className="workspace-core__orbit" aria-hidden="true">{coreSystems.map((system) => <span key={system}><i />{system}</span>)}</div></aside>
      <button className="workspace-nova-dock" type="button" onClick={() => setActiveView("conversations")}><Sparkles size={20} /><span>Demander à Nova dans ce Workspace…</span><ArrowUpRight size={18} /></button>
    </GlassPanel>
    ...
    <aside className="workspace-home__rail" aria-label="Contexte intelligent du Workspace">
      ...
      <GlassPanel className={`workspace-rail-status workspace-rail-status--${overviewState}`}><span>TRIDENT AI</span><strong>{activeWorkspace?.name || "Workspace"}</strong><p aria-live="polite"><i /> {overviewState === "ready" ? "Contexte synchronisé" : overviewState === "loading" ? "Synchronisation…" : "Contexte indisponible"}</p><small>Les données affichées restent limitées à ce Workspace.</small></GlassPanel>
    </aside>
  </section>;
}
