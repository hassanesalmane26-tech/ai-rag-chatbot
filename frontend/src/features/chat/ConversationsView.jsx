...
import { BookOpen, Brain, Files, LoaderCircle, MessageSquarePlus, RefreshCw, Send, Sparkles } from "lucide-react";
...
export default function ConversationsView({ workspaceId }) {
  ...
  return <section className="conversation-layout">
    ...
    <section className={`conversation-panel ${activeConversation ? "conversation-panel--active" : "conversation-panel--ready"}`}>
      {activeConversation ? <>
      ...
      </> : <div className="nova-ready"><div className="nova-ready__field" aria-hidden="true"><i /><i /><i /></div><div className="nova-ready__identity"><Sparkles size={30} /><span>NOVA · INTELLIGENCE DU WORKSPACE</span><h2>Prête dans {activeWorkspace?.name || "votre Workspace"}</h2><p>Ouvrez une conversation pour analyser, écrire ou interroger le contexte autorisé de cet environnement.</p></div><div className="nova-ready__context" role="list" aria-label="Systèmes de contexte disponibles dans le Workspace"><span role="listitem"><BookOpen size={16} />Knowledge</span><span role="listitem"><Brain size={16} />Memory</span><span role="listitem"><Files size={16} />Fichiers</span></div>{error && <p className="inline-error" role="alert">{error}</p>}<button className="nova-ready__action" type="button" onClick={addConversation} disabled={creating}>{creating ? "Création…" : "Démarrer avec Nova"}</button></div>}
    </section>
  </section>;
}
