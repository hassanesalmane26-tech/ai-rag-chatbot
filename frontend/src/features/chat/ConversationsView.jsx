...
import { BookOpen, Brain, Files, LoaderCircle, MessageSquarePlus, RefreshCw, Send } from "lucide-react";
...
export default function ConversationsView({ workspaceId }) {
  ...
  return <section className="conversation-layout">
    ...
    <section className={`conversation-panel ${activeConversation ? "conversation-panel--active" : "conversation-panel--ready"}`} aria-label="Nova">
      ...
      </> : <div className="nova-ready"><div className="nova-ready__field" aria-hidden="true"><i /><i /><i /></div><div className="nova-ready__identity"><TridentMark /><span>NOVA · INTELLIGENCE DU WORKSPACE</span><h2>Bienvenue dans {activeWorkspace?.name || "votre Workspace"}</h2><p>Nova est prête à travailler avec le contexte autorisé de cet environnement.</p></div><div className="nova-ready__context" role="list" aria-label="Systèmes de contexte disponibles dans le Workspace"><span role="listitem"><BookOpen size={16} />Knowledge</span><span role="listitem"><Brain size={16} />Memory</span><span role="listitem"><Files size={16} />Fichiers</span></div><div className="nova-ready__suggestions" aria-label="Suggestions"><button type="button" onClick={() => setText("Que peux-tu m’aider à accomplir dans ce Workspace ?")}>Explorer ce Workspace</button><button type="button" onClick={() => setText("Résume le contexte disponible dans Knowledge.")}>Interroger Knowledge</button><button type="button" onClick={() => setText("Démarrons une nouvelle conversation.")}>Parler avec Nova</button></div>{error && <p className="inline-error" role="alert">{error}</p>}</div>}
