...
export default function FilesView() {
  ...
  async function download(document) {
    ...
    } catch (cause) { setDownloadError(cause.message); }
    finally { setDownloadingId(null); }
  }
  return <section className="product-view files-view" aria-labelledby="files-title">
    <header className="product-view__header"><div><span>RESSOURCES DU WORKSPACE</span><h2 id="files-title">Fichiers</h2><p>Les fichiers originaux privés de {activeWorkspace?.name}. Knowledge permet à Nova d’en exploiter le contenu.</p></div><button type="button" className="view-refresh" onClick={refresh} aria-label="Actualiser les fichiers"><RefreshCw size={17} /></button></header>
