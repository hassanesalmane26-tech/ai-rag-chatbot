...
export default function Sidebar() {
  ...
  return <aside className="sidebar" aria-label="Navigation du Workspace">
    ...
    <nav className="sidebar-menu" aria-label="Modules principaux">{workspaceModules.filter((item) => item.section === "primary").map(({ id, label, icon: Icon, mobile }) => <button key={id} title={label} className={`menu-item ds-nav-control ${mobile ? "menu-item--mobile" : ""} ${activeView === id ? "active" : ""}`} aria-current={activeView === id ? "page" : undefined} onClick={() => setActiveView(id)}><Icon size={17} />{label}</button>)}</nav>
    <div className="sidebar-section-label sidebar-section-label--workspaces">WORKSPACES</div>
    ...
    <nav className="sidebar-menu sidebar-menu--secondary" aria-label="Workspace et paramètres">{workspaceModules.filter((item) => item.section === "secondary").map(({ id, label, icon: Icon }) => <button key={id} title={label} className={`menu-item ds-nav-control ${activeView === id ? "active" : ""}`} aria-current={activeView === id ? "page" : undefined} onClick={() => setActiveView(id)}><Icon size={17} />{label}</button>)}</nav>
